...
mxCellRenderer.registerShape(mxShapeBasicCross.prototype.cst.CROSS,
    mxShapeBasicCross);
...
Graph.handleFactory[mxShapeBasicCross.prototype.cst.CROSS] = function(state) {
    var handles = [
        Graph.createHandle(state, ['dx'], function(bounds) {
            var dx = Math.max(0, Math.min(bounds.width / 2, bounds.width / 2,
                parseFloat(mxUtils.getValue(this.state.style, 'dx', this.dx))));

            return new mxPoint(bounds.x + bounds.width / 2 + dx,
                bounds.y + bounds.height / 2 - dx);
        }, function(bounds, pt) {
            this.state.style['dx'] = Math.round(100 * Math.max(0,
                        Math.min(bounds.height / 2, bounds.width / 2,
                            pt.x - bounds.x - bounds.width / 2))) / 100;
        })];
...
mxShapeBasicRectCallout.prototype.getLabelMargins = function() {
    if (mxUtils.getValue(this.style, 'boundedLbl', false)) {
        return new mxRectangle(0, 0, 0,
            parseFloat(mxUtils.getValue(this.style, 'dy', this.dy)) *
            this.scale);
    }
...
mxCellRenderer.registerShape(mxShapeBasicRectCallout.prototype.cst.RECT_CALLOUT,
...
Graph.handleFactory[mxShapeBasicRectCallout.prototype.cst.RECT_CALLOUT] = function(state) {
    var handles = [
        Graph.createHandle(state, ['dx', 'dy'], function(bounds) {
            var dx = Math.max(0, Math.min(bounds.width,
                parseFloat(mxUtils.getValue(this.state.style, 'dx', this.dx))));
            var dy = Math.max(0, Math.min(bounds.height,
                parseFloat(mxUtils.getValue(this.state.style, 'dy', this.dy))));

            return new mxPoint(bounds.x + dx, bounds.y + bounds.height - dy);
        }, function(bounds, pt) {
            var y = parseFloat(
                    mxUtils.getValue(this.state.style, 'dy', this.dy)) * 0.6;
            this.state.style['dx'] = Math.round(100 *
                    Math.max(y, Math.min(bounds.width - y, pt.x - bounds.x))) /
                100;
            this.state.style['dy'] = Math.round(Math.max(0,
 ...
 * Paints the vertex shape.
 */
mxShapeBasicRoundRectCallout.prototype.paintVertexShape = function(
    c, x, y, w, h) {
    c.translate(x, y);
...
mxCellRenderer.registerShape(
    mxShapeBasicRoundRectCallout.prototype.cst.ROUND_RECT_CALLOUT,
    mxShapeBasicRoundRectCallout);
...
Graph.handleFactory[mxShapeBasicRoundRectCallout.prototype.cst.ROUND_RECT_CALLOUT] = function(state) {
    return [
        Graph.createHandle(state, ['dx', 'dy'], function(bounds) {
            ...
            var size = Math.max(0, Math.min(bounds.width, parseFloat(
                mxUtils.getValue(this.state.style, 'size', this.size))));

            return new mxPoint(bounds.x + bounds.width - size, bounds.y + 10);
        }, function(bounds, pt) {
            var dy = parseFloat(
                mxUtils.getValue(this.state.style, 'dy', this.dy));
            this.state.style['size'] = Math.round(100 * Math.max(0,
                        Math.min(bounds.width / 2, (bounds.height - dy) / 2,
                            bounds.x + bounds.width - pt.x))) / 100;
        })];
};
...
mxShapeBasicWave.prototype.paintVertexShape = function(c, x, y, w, h) {
    ...
    var dy = h * Math.max(0, Math.min(h,
            parseFloat(mxUtils.getValue(this.style, 'dy', this.dy))));
    var fy = 1.4;
...
mxCellRenderer.registerShape(mxShapeBasicWave.prototype.cst.WAVE,
    mxShapeBasicWave);
...
Graph.handleFactory[mxShapeBasicWave.prototype.cst.WAVE] = function(state) {
    var handles = [
        Graph.createHandle(state, ['dy'], function(bounds) {
            var dy = Math.max(0, Math.min(1,
                parseFloat(mxUtils.getValue(this.state.style, 'dy', this.dy))));

            return new mxPoint(bounds.x + bounds.width / 2,
                bounds.y + dy * bounds.height);
        }, function(bounds, pt) {
            this.state.style['dy'] = Math.round(100 * Math.max(0,
                        Math.min(1, (pt.y - bounds.y) / bounds.height))) / 100;
        })];
...
mxShapeBasicOctagon.prototype.paintVertexShape = function(c, x, y, w, h) {
    ...
    var dx = Math.max(0, Math.min(w,
            parseFloat(mxUtils.getValue(this.style, 'dx', this.dx)))) * 2;
...
mxCellRenderer.registerShape(mxShapeBasicOctagon.prototype.cst.OCTAGON,
    mxShapeBasicOctagon);
...
Graph.handleFactory[mxShapeBasicOctagon.prototype.cst.OCTAGON] = function(state) {
    var handles = [
        Graph.createHandle(state, ['dx'], function(bounds) {
            var dx = Math.max(0, Math.min(bounds.width / 4, bounds.width / 4,
                parseFloat(mxUtils.getValue(this.state.style, 'dx', this.dx))));

            return new mxPoint(bounds.x + dx, bounds.y + dx);
        }, function(bounds, pt) {
            this.state.style['dx'] = Math.round(100 * Math.max(0,
                        Math.min(bounds.height / 4, bounds.width / 4,
                            pt.x - bounds.x))) / 100;
        })];
...
mxShapeBasicIsoCube.prototype.paintVertexShape = function(c, x, y, w, h) {
    ...
    var isoAngle = Math.max(0.01, Math.min(94, parseFloat(
            mxUtils.getValue(this.style, 'isoAngle', this.isoAngle)))) *
        Math.PI / 200;
...
mxCellRenderer.registerShape(mxShapeBasicIsoCube.prototype.cst.ISO_CUBE,
    mxShapeBasicIsoCube);
...
Graph.handleFactory[mxShapeBasicIsoCube.prototype.cst.ISO_CUBE] = function(state) {
    var handles = [
        Graph.createHandle(state, ['isoAngle'], function(bounds) {
            var isoAngle = Math.max(0.01, Math.min(94, parseFloat(
                    mxUtils.getValue(this.state.style, 'isoAngle',
                        this.isoAngle)))) * Math.PI / 200;
            var isoH = Math.min(bounds.width * Math.tan(isoAngle),
                bounds.height * 0.5);

            return new mxPoint(bounds.x, bounds.y + isoH);
        }, function(bounds, pt) {
            this.state.style['isoAngle'] = Math.round(
                    100 * Math.max(0, Math.min(100, pt.y - bounds.y))) / 100;
        })];
...
mxShapeBasicTriangleAcute.prototype.paintVertexShape = function(c, x, y, w, h) {
    ...
    var dx = w * Math.max(0, Math.min(w,
            parseFloat(mxUtils.getValue(this.style, 'dx', this.dx))));
...
mxCellRenderer.registerShape(
    mxShapeBasicTriangleAcute.prototype.cst.ACUTE_TRIANGLE,
    mxShapeBasicTriangleAcute);
...
Graph.handleFactory[mxShapeBasicTriangleAcute.prototype.cst.ACUTE_TRIANGLE] = function(state) {
    var handles = [
        Graph.createHandle(state, ['dx'], function(bounds) {
            var dx = Math.max(0, Math.min(1,
                parseFloat(mxUtils.getValue(this.state.style, 'dx', this.dx))));

            return new mxPoint(bounds.x + dx * bounds.width, bounds.y + 10);
        }, function(bounds, pt) {
            this.state.style['dx'] = Math.round(100 * Math.max(0,
                        Math.min(1, (pt.x - bounds.x) / bounds.width))) / 100;
        })];
 ...
 * Paints the vertex shape.
 */
mxShapeBasicTriangleObtuse.prototype.paintVertexShape = function(
    c, x, y, w, h) {
    c.translate(x, y);
...
mxCellRenderer.registerShape(
    mxShapeBasicTriangleObtuse.prototype.cst.OBTUSE_TRIANGLE,
    mxShapeBasicTriangleObtuse);
...
Graph.handleFactory[mxShapeBasicTriangleObtuse.prototype.cst.OBTUSE_TRIANGLE] = function(state) {
    var handles = [
        Graph.createHandle(state, ['dx'], function(bounds) {
            ...
            return new mxPoint(bounds.x + dx * bounds.width,
...
mxCellRenderer.registerShape(mxShapeBasicDrop.prototype.cst.DROP,
    mxShapeBasicDrop);
...
mxCellRenderer.registerShape(mxShapeBasicCone2.prototype.cst.CONE2,
    mxShapeBasicCone2);
...
Graph.handleFactory[mxShapeBasicCone2.prototype.cst.CONE2] = function(state) {
    ...
    var handle2 = Graph.createHandle(state, ['dy'], function(bounds) {
        ...
        this.state.style['dy'] = Math.round(100 *
                Math.max(0, Math.min(1, (pt.y - bounds.y) / bounds.height))) /
            100;
    });
...
mxShapeBasicPyramid.prototype.paintVertexShape = function(c, x, y, w, h) {
    ...
    var dx1 = w * Math.max(0, Math.min(w,
            parseFloat(mxUtils.getValue(this.style, 'dx1', this.dx1))));
    var dx2 = w * Math.max(0, Math.min(w,
            parseFloat(mxUtils.getValue(this.style, 'dx2', this.dx2))));
    var dy1 = h * Math.max(0, Math.min(h,
            parseFloat(mxUtils.getValue(this.style, 'dy1', this.dy1))));
    var dy2 = h * Math.max(0, Math.min(h,
            parseFloat(mxUtils.getValue(this.style, 'dy2', this.dy2))));
...
mxCellRenderer.registerShape(mxShapeBasicPyramid.prototype.cst.PYRAMID,
    mxShapeBasicPyramid);
...
Graph.handleFactory[mxShapeBasicPyramid.prototype.cst.PYRAMID] = function(state) {
    var handles = [
        Graph.createHandle(state, ['dx1'], function(bounds) {
            var dx1 = Math.max(0, Math.min(1, parseFloat(
                mxUtils.getValue(this.state.style, 'dx1', this.dx1))));

            return new mxPoint(bounds.x + dx1 * bounds.width, bounds.y + 10);
        }, function(bounds, pt) {
            this.state.style['dx1'] = Math.round(100 * Math.max(0,
                        Math.min(1, (pt.x - bounds.x) / bounds.width))) / 100;
        })];

    var handle2 = Graph.createHandle(state, ['dx2'], function(bounds) {
        var dx2 = Math.max(0, Math.min(1,
            parseFloat(mxUtils.getValue(this.state.style, 'dx2', this.dx2))));

        return new mxPoint(bounds.x + dx2 * bounds.width,
            bounds.y + bounds.height - 10);
    }, function(bounds, pt) {
        this.state.style['dx2'] = Math.round(100 *
    ...
    var handle3 = Graph.createHandle(state, ['dy1'], function(bounds) {
        var dy1 = Math.max(0, Math.min(1,
            parseFloat(mxUtils.getValue(this.state.style, 'dy1', this.dy1))));

        return new mxPoint(bounds.x + 10, bounds.y + dy1 * bounds.height);
    }, function(bounds, pt) {
        this.state.style['dy1'] = Math.round(100 *
                Math.max(0, Math.min(1, (pt.y - bounds.y) / bounds.height))) /
            100;
    });
    ...
    var handle4 = Graph.createHandle(state, ['dy2'], function(bounds) {
        var dy2 = Math.max(0, Math.min(1,
            parseFloat(mxUtils.getValue(this.state.style, 'dy2', this.dy2))));

        return new mxPoint(bounds.x + bounds.width - 10,
            bounds.y + dy2 * bounds.height);
    }, function(bounds, pt) {
        this.state.style['dy2'] = Math.round(100 *
                Math.max(0, Math.min(1, (pt.y - bounds.y) / bounds.height))) /
            100;
    });
 ...
 * Extends mxShape.
 */
function mxShapeBasic4PointStar(bounds, fill, stroke, strokewidth) {
    mxShape.call(this);
    this.bounds = bounds;
    this.fill = fill;
    this.stroke = stroke;
 ...
 * Extends mxShape.
 */
mxUtils.extend(mxShapeBasic4PointStar, mxActor);

mxShapeBasic4PointStar.prototype.customProperties = [
    {
        name: 'dx',
        dispName: 'Thickness',
        type: 'float',
...
mxShapeBasic4PointStar.prototype.cst = {FOUR_POINT_STAR: 'mxgraph.basic.4_point_star'};
/**
 * Function: paintVertexShape
 *
 * Paints the vertex shape.
 */
mxShapeBasic4PointStar.prototype.paintVertexShape = function(
    c, x, y, w, h) {
    c.translate(x, y);

    var dx = 0.5 * Math.max(0, Math.min(w,
            parseFloat(mxUtils.getValue(this.style, 'dx', this.dx))));
    c.begin();
    c.moveTo(0, h * 0.5);
    c.lineTo(dx * w, dx * h);
    ...
    mxShapeBasic4PointStar.prototype.constraints = [
        new mxConnectionConstraint(new mxPoint(0.5, 0), true),
        new mxConnectionConstraint(new mxPoint(0.5, 1), true),
        new mxConnectionConstraint(new mxPoint(0, 0.5), true),
        new mxConnectionConstraint(new mxPoint(1, 0.5), true),
        new mxConnectionConstraint(new mxPoint(dx, dx), false),
        new mxConnectionConstraint(new mxPoint(1 - dx, dx), false),
        new mxConnectionConstraint(new mxPoint(dx, 1 - dx), false),
        new mxConnectionConstraint(new mxPoint(1 - dx, 1 - dx), false),
    ];
};

mxCellRenderer.registerShape(
    mxShapeBasic4PointStar.prototype.cst.FOUR_POINT_STAR,
    mxShapeBasic4PointStar);

Graph.handleFactory[mxShapeBasic4PointStar.prototype.cst.FOUR_POINT_STAR] = function(state) {
    var handles = [
        Graph.createHandle(state, ['dx'], function(bounds) {
            var dx = Math.max(0, Math.min(1,
                parseFloat(
                    mxUtils.getValue(this.state.style, 'dx', this.dx))));

            var point = new mxPoint(bounds.x + dx * bounds.width / 2,
                bounds.y + dx * bounds.height / 2);

            return point;
        }, function(bounds, pt) {
            this.state.style['dx'] = Math.round(100 * Math.max(0,
                        Math.min(1,
                            2 * (pt.x - bounds.x) / bounds.width))) /
                100;
        })];
 ...
 * Extends mxShape.
 */
function mxShapeBasic3PointStar(bounds, fill, stroke, strokewidth) {
    mxShape.call(this);
    this.bounds = bounds;
    this.fill = fill;
    this.stroke = stroke;
 ...
 * Extends mxShape.
 */
mxUtils.extend(mxShapeBasic3PointStar, mxActor);

mxShapeBasic3PointStar.prototype.customProperties = [
    {
        name: 'dx',
        dispName: 'Thickness',
        type: 'float',
...
mxShapeBasic3PointStar.prototype.cst = {THREE_POINT_STAR: 'mxgraph.basic.3_point_star'};
/**
 * Function: paintVertexShape
 *
 * Paints the vertex shape.
 */
mxShapeBasic3PointStar.prototype.paintVertexShape = function(c, x, y, w, h) {
    c.translate(x, y);
    ...
    c.begin();
    c.moveTo(rw, 0);
    c.lineTo(rw + drw * Math.cos(30 * Math.PI / 180),
        rh - drh * Math.sin(30 * Math.PI / 180));
    c.lineTo(rw + rw * Math.cos(30 * Math.PI / 180),
        rh + rh * Math.sin(30 * Math.PI / 180));
    c.lineTo(rw, rh + drh);
    c.lineTo(rw - rw * Math.cos(30 * Math.PI / 180),
        rh + rh * Math.sin(30 * Math.PI / 180));
    c.lineTo(rw - drw * Math.cos(30 * Math.PI / 180),
    ...
    mxShapeBasic3PointStar.prototype.constraints = [
        new mxConnectionConstraint(new mxPoint(0.5, 0), true),
        new mxConnectionConstraint(new mxPoint((rw + drw * Math.cos(30 * Math.PI / 180)) / w, (rh - drh * Math.sin(30 * Math.PI / 180)) / h),false),
        new mxConnectionConstraint(new mxPoint((rw + rw * Math.cos(30 * Math.PI / 180)) / w, (rh + rh * Math.sin(30 * Math.PI / 180)) / h),false),
        new mxConnectionConstraint(new mxPoint(0.5, (rh + drh) / h), false),
        new mxConnectionConstraint(new mxPoint(1 - ((rw + rw * Math.cos(30 * Math.PI / 180)) / w), (rh + rh * Math.sin(30 * Math.PI / 180)) / h),false),
        new mxConnectionConstraint(new mxPoint(1 - ((rw + drw * Math.cos(30 * Math.PI / 180)) / w), (rh - drh * Math.sin(30 * Math.PI / 180)) / h),false)
    ];
};

mxCellRenderer.registerShape(
    mxShapeBasic3PointStar.prototype.cst.THREE_POINT_STAR,
    mxShapeBasic3PointStar);

Graph.handleFactory[mxShapeBasic3PointStar.prototype.cst.THREE_POINT_STAR] = function(state) {
    var handles = [
        Graph.createHandle(state, ['dx'], function(bounds) {
            var dx = Math.max(0, Math.min(1,
                parseFloat(mxUtils.getValue(this.state.style, 'dx', this.dx))));

            var rw = bounds.width * 0.5,
                rh = bounds.height * 0.5,
                drw = rw - rw * dx,
                drh = rh - rh * dx;

            return new mxPoint(
                bounds.x + (rw - drw * Math.cos(30 * Math.PI / 180)),
                bounds.y + (rh - drh * Math.sin(30 * Math.PI / 180)));
        }, function(bounds, pt) {
            this.state.style['dx'] = Math.round(100 * Math.max(0,
                        Math.min(1, 2 * (pt.x - bounds.x) / bounds.width))) /
                100;
        })];
...
mxCellRenderer.registerShape(
    mxShapeBasicDiagSnipRect.prototype.cst.DIAG_SNIP_RECT,
    mxShapeBasicDiagSnipRect);
...
mxCellRenderer.registerShape(
    mxShapeBasicDiagRoundRect.prototype.cst.DIAG_ROUND_RECT,
 ...
 * Paints the vertex shape.
 */
mxShapeBasicCornerRoundRect.prototype.paintVertexShape = function(
    c, x, y, w, h) {
    c.translate(x, y);
...
mxCellRenderer.registerShape(
    mxShapeBasicCornerRoundRect.prototype.cst.CORNER_ROUND_RECT,
    mxShapeBasicCornerRoundRect);
...
mxCellRenderer.registerShape(mxShapeBasicPlaque.prototype.cst.PLAQUE,
    mxShapeBasicPlaque);
...
mxCellRenderer.registerShape(mxShapeBasicFrame.prototype.cst.FRAME,
    mxShapeBasicFrame);
...
mxCellRenderer.registerShape(mxShapeBasicPlaqueFrame.prototype.cst.PLAQUE_FRAME,
    mxShapeBasicPlaqueFrame);
...
mxCellRenderer.registerShape(
    mxShapeBasicRoundedFrame.prototype.cst.ROUNDED_FRAME,
    mxShapeBasicRoundedFrame);
...
mxCellRenderer.registerShape(mxShapeBasicFrameCorner.prototype.cst.FRAME_CORNER,
...
mxCellRenderer.registerShape(mxShapeBasicDiagStripe.prototype.cst.DIAG_STRIPE,
...
Graph.handleFactory[mxShapeBasicDiagStripe.prototype.cst.DIAG_STRIPE] = function(state) {
    var handles = [
        Graph.createHandle(state, ['dx'], function(bounds) {
            var dx = Math.max(0, Math.min(bounds.width / 2,
                parseFloat(mxUtils.getValue(this.state.style, 'dx', this.dx))));
...
mxCellRenderer.registerShape(mxShapeBasicDonut.prototype.cst.DONUT,
    mxShapeBasicDonut);
...
mxCellRenderer.registerShape(mxShapeBasicLayeredRect.prototype.cst.LAYERED_RECT,
    mxShapeBasicLayeredRect);
...
Graph.handleFactory[mxShapeBasicLayeredRect.prototype.cst.LAYERED_RECT] = function(state) {
    var handles = [
        Graph.createHandle(state, ['dx'], function(bounds) {
            ...
            return new mxPoint(bounds.x + bounds.width - dx,
                bounds.y + bounds.height - dx);
        }, function(bounds, pt) {
            this.state.style['dx'] = Math.round(100 * Math.max(0,
                        Math.min(bounds.height / 2, bounds.width / 2,
                            -pt.x + bounds.width + bounds.x))) / 100;
        })];
...
mxCellRenderer.registerShape(mxShapeBasicButton.prototype.cst.BUTTON,
    mxShapeBasicButton);
...
mxCellRenderer.registerShape(
    mxShapeBasicShadedButton.prototype.cst.SHADED_BUTTON,
    mxShapeBasicShadedButton);
...
mxShapeBasicPie.prototype.paintVertexShape = function(c, x, y, w, h) {
    ...
    var startAngle = 2 * Math.PI * Math.max(0, Math.min(1, parseFloat(
            mxUtils.getValue(this.style, 'startAngle', this.startAngle))));
    var endAngle = 2 * Math.PI * Math.max(0, Math.min(1, parseFloat(
            mxUtils.getValue(this.style, 'endAngle', this.endAngle))));
    var rx = w * 0.5;
    var ry = h * 0.5;
...
mxCellRenderer.registerShape(mxShapeBasicPie.prototype.cst.PIE,
    mxShapeBasicPie);
...
Graph.handleFactory[mxShapeBasicPie.prototype.cst.PIE] = function(state) {
    var handles = [
        Graph.createHandle(state, ['startAngle'], function(bounds) {
            var startAngle = 2 * Math.PI * Math.max(0, Math.min(1, parseFloat(
                    mxUtils.getValue(this.state.style, 'startAngle',
                        this.startAngle))));

            return new mxPoint(bounds.x + bounds.width * 0.5 +
                Math.sin(startAngle) * bounds.width * 0.5,
                bounds.y + bounds.height * 0.5 -
                Math.cos(startAngle) * bounds.height * 0.5);
        }, function(bounds, pt) {
            var handleX = Math.round(100 * Math.max(-1, Math.min(1,
                        (pt.x - bounds.x - bounds.width * 0.5) /
                        (bounds.width * 0.5)))) / 100;
            var handleY = -Math.round(100 * Math.max(-1, Math.min(1,
                        (pt.y - bounds.y - bounds.height * 0.5) /
                        (bounds.height * 0.5)))) / 100;
    ...
    var handle2 = Graph.createHandle(state, ['endAngle'], function(bounds) {
        var endAngle = 2 * Math.PI * Math.max(0, Math.min(1, parseFloat(
                mxUtils.getValue(this.state.style, 'endAngle',
                    this.endAngle))));

        return new mxPoint(bounds.x + bounds.width * 0.5 +
            Math.sin(endAngle) * bounds.width * 0.5,
            bounds.y + bounds.height * 0.5 -
            Math.cos(endAngle) * bounds.height * 0.5);
    }, function(bounds, pt) {
        var handleX = Math.round(100 * Math.max(-1, Math.min(1,
                    (pt.x - bounds.x - bounds.width * 0.5) /
                    (bounds.width * 0.5)))) / 100;
...
mxCellRenderer.registerShape(mxShapeBasicArc.prototype.cst.ARC,
    mxShapeBasicArc);
 ...
 * Paints the vertex shape.
 */
mxShapeBasicPartConcEllipse.prototype.paintVertexShape = function(
    c, x, y, w, h) {
    c.translate(x, y);
    ...
    var endAngle = 2 * Math.PI * Math.max(0, Math.min(1, parseFloat(
            mxUtils.getValue(this.style, 'endAngle', this.endAngle))));
    var arcWidth = 1 - Math.max(0, Math.min(1, parseFloat(
            mxUtils.getValue(this.style, 'arcWidth', this.arcWidth))));
    var rx = w * 0.5;
    var ry = h * 0.5;
    var rx2 = rx * arcWidth;
    var ry2 = ry * arcWidth;
...
mxCellRenderer.registerShape(
    mxShapeBasicPartConcEllipse.prototype.cst.PART_CONC_ELLIPSE,
...
Graph.handleFactory[mxShapeBasicPartConcEllipse.prototype.cst.PART_CONC_ELLIPSE] = function(state) {
    ...
    var handle3 = Graph.createHandle(state, ['arcWidth'], function(bounds) {
        var arcWidth = Math.max(0, Math.min(1, parseFloat(
            mxUtils.getValue(this.state.style, 'arcWidth', this.arcWidth))));

        return new mxPoint(bounds.x + bounds.width / 2,
            bounds.y + arcWidth * bounds.height * 0.5);
    }, function(bounds, pt) {
        this.state.style['arcWidth'] = Math.round(100 * Math.max(0,
                    Math.min(bounds.height / 2, bounds.width / 2,
                        (pt.y - bounds.y) / (bounds.height * 0.5)))) / 100;
    });
...
mxShapeBasicNumEntryVert.prototype.paintVertexShape = function(c, x, y, w, h) {
    ...
    var dy = Math.max(0,
        Math.min(w, parseFloat(mxUtils.getValue(this.style, 'dy', this.dy))));
    ...
    c.arcTo(d * 0.5 + inset, d * 0.5 + inset, 0, 0, 0,
        w * 0.5 + d * 0.5 + inset, d * 0.5);
    c.lineTo(w, d * 0.5);
    c.lineTo(w, h);
    c.lineTo(0, h);
    c.close();
...
mxCellRenderer.registerShape(
    mxShapeBasicNumEntryVert.prototype.cst.NUM_ENTRY_VERT,
    mxShapeBasicNumEntryVert);
...
Graph.handleFactory[mxShapeBasicNumEntryVert.prototype.cst.NUM_ENTRY_VERT] = function(state) {
    var handles = [
        Graph.createHandle(state, ['dy'], function(bounds) {
            var dy = Math.max(0, Math.min(bounds.width, bounds.width,
                parseFloat(mxUtils.getValue(this.state.style, 'dy', this.dy))));
...
mxCellRenderer.registerShape(mxShapeBasicBendingArch.prototype.cst.BENDING_ARCH,
    mxShapeBasicBendingArch);
 ...
 * Paints the vertex shape.
 */
mxShapeBasicThreeCornerRoundRect.prototype.paintVertexShape = function(
    c, x, y, w, h) {
    c.translate(x, y);
...
mxCellRenderer.registerShape(
    mxShapeBasicThreeCornerRoundRect.prototype.cst.THREE_CORNER_ROUND_RECT,
...
mxShapeBasicRect2.prototype.paintVertexShape = function(c, x, y, w, h) {
    ...
    var absoluteCornerSize = mxUtils.getValue(this.style, 'absoluteCornerSize',
        this.absoluteCornerSize);

    var size = Math.max(0, Math.min(w,
        parseFloat(mxUtils.getValue(this.style, 'size', this.size))));
    ...
    var rectOutline = mxUtils.getValue(this.style, 'rectOutline',
        this.rectOutline);
    var indent = Math.max(0, Math.min(w,
        parseFloat(mxUtils.getValue(this.style, 'indent', this.indent))));
...
//	indent = Math.min(indent, 2 * size, Math.min(w, h) - size);
    ...
    var topRightStyle = mxUtils.getValue(this.style, 'topRightStyle',
        'default');
    var bottomRightStyle = mxUtils.getValue(this.style, 'bottomRightStyle',
        'default');
    var bottomLeftStyle = mxUtils.getValue(this.style, 'bottomLeftStyle',
        'default');
    ...
    if ((top || right || bottom || left) && rectOutline != 'frame') {
        ...
        if (!top) {
            ...
            mxShapeBasicRect2.prototype.moveNW(c, x, y, w, h, rectStyle,
                topLeftStyle, size, left);
        }

        if (top) {
            mxShapeBasicRect2.prototype.paintNW(c, x, y, w, h, rectStyle,
                topLeftStyle, size, left);
        }

        mxShapeBasicRect2.prototype.paintTop(c, x, y, w, h, rectStyle,
            topRightStyle, size, right);

        if (right) {
            mxShapeBasicRect2.prototype.paintNE(c, x, y, w, h, rectStyle,
                topRightStyle, size, top);
        }

        mxShapeBasicRect2.prototype.paintRight(c, x, y, w, h, rectStyle,
            bottomRightStyle, size, bottom);

        if (bottom) {
            mxShapeBasicRect2.prototype.paintSE(c, x, y, w, h, rectStyle,
                bottomRightStyle, size, right);
        }

        mxShapeBasicRect2.prototype.paintBottom(c, x, y, w, h, rectStyle,
            bottomLeftStyle, size, left);

        if (left) {
            mxShapeBasicRect2.prototype.paintSW(c, x, y, w, h, rectStyle,
                bottomLeftStyle, size, bottom);
        }

        mxShapeBasicRect2.prototype.paintLeft(c, x, y, w, h, rectStyle,
            topLeftStyle, size, top);
        c.close();
        c.fill();
        ...
        if (!top) {
            ...
            mxShapeBasicRect2.prototype.moveNWInner(c, x, y, w, h, rectStyle,
                topLeftStyle, size, indent, top, left);
        }

        mxShapeBasicRect2.prototype.paintLeftInner(c, x, y, w, h, rectStyle,
            bottomLeftStyle, size, indent, bottom, left);

        if (left && bottom) {
            mxShapeBasicRect2.prototype.paintSWInner(c, x, y, w, h, rectStyle,
                bottomLeftStyle, size, indent, bottom);
        }

        mxShapeBasicRect2.prototype.paintBottomInner(c, x, y, w, h, rectStyle,
            bottomRightStyle, size, indent, right, bottom);

        if (bottom && right) {
            mxShapeBasicRect2.prototype.paintSEInner(c, x, y, w, h, rectStyle,
                bottomRightStyle, size, indent);
        }

        mxShapeBasicRect2.prototype.paintRightInner(c, x, y, w, h, rectStyle,
            topRightStyle, size, indent, top, right);

        if (right && top) {
            mxShapeBasicRect2.prototype.paintNEInner(c, x, y, w, h, rectStyle,
                topRightStyle, size, indent);
        }

        mxShapeBasicRect2.prototype.paintTopInner(c, x, y, w, h, rectStyle,
            topLeftStyle, size, indent, left, top);

        if (top && left) {
            mxShapeBasicRect2.prototype.paintNWInner(c, x, y, w, h, rectStyle,
                topLeftStyle, size, indent);
        }
        ...
        if (fillColor == 'none') {
            c.begin();
            mxShapeBasicRect2.prototype.paintFolds(c, x, y, w, h, rectStyle,
                topLeftStyle, topRightStyle, bottomRightStyle, bottomLeftStyle,
    ...
    if (!top && !right && !bottom && left) {
        ...
        if (rectOutline != 'frame') {
            c.begin();
            mxShapeBasicRect2.prototype.moveSW(c, x, y, w, h, rectStyle,
                topLeftStyle, size, bottom);
            mxShapeBasicRect2.prototype.paintLeft(c, x, y, w, h, rectStyle,
                topLeftStyle, size, top);

            if (rectOutline == 'double') {
                mxShapeBasicRect2.prototype.moveNWInner(c, x, y, w, h,
                    rectStyle, topLeftStyle, size, indent, top, left);
                mxShapeBasicRect2.prototype.paintLeftInner(c, x, y, w, h,
                    rectStyle, bottomLeftStyle, size, indent, bottom, left);
            }
            ...
            mxShapeBasicRect2.prototype.paintLeft(c, x, y, w, h, rectStyle,
                topLeftStyle, size, top);
            mxShapeBasicRect2.prototype.lineNWInner(c, x, y, w, h, rectStyle,
                topLeftStyle, size, indent, top, left);
            mxShapeBasicRect2.prototype.paintLeftInner(c, x, y, w, h, rectStyle,
                bottomLeftStyle, size, indent, bottom, left);
            c.close();
        ...
        if (rectOutline != 'frame') {
            c.begin();
            mxShapeBasicRect2.prototype.moveSE(c, x, y, w, h, rectStyle,
                bottomRightStyle, size, right);
            mxShapeBasicRect2.prototype.paintBottom(c, x, y, w, h, rectStyle,
                bottomLeftStyle, size, left);

            if (rectOutline == 'double') {
                mxShapeBasicRect2.prototype.moveSWInner(c, x, y, w, h,
                    rectStyle, bottomLeftStyle, size, indent, left);
                mxShapeBasicRect2.prototype.paintBottomInner(c, x, y, w, h,
                    rectStyle, bottomRightStyle, size, indent, right, bottom);
            }
            ...
            mxShapeBasicRect2.prototype.paintBottom(c, x, y, w, h, rectStyle,
                bottomLeftStyle, size, left);
            mxShapeBasicRect2.prototype.lineSWInner(c, x, y, w, h, rectStyle,
                bottomLeftStyle, size, indent, left);
            mxShapeBasicRect2.prototype.paintBottomInner(c, x, y, w, h,
                rectStyle, bottomRightStyle, size, indent, right, bottom);
            c.close();
            c.fillAndStroke();
        ...
        if (rectOutline != 'frame') {
            ...
            if (rectOutline == 'double') {
                ...
                mxShapeBasicRect2.prototype.paintLeftInner(c, x, y, w, h,
                    rectStyle, bottomLeftStyle, size, indent, bottom, left);
                mxShapeBasicRect2.prototype.paintSWInner(c, x, y, w, h,
                    rectStyle, bottomLeftStyle, size, indent, bottom);
                mxShapeBasicRect2.prototype.paintBottomInner(c, x, y, w, h,
                    rectStyle, bottomRightStyle, size, indent, right, bottom);
            }
        ...
        if (rectOutline != 'frame') {
            c.begin();
            mxShapeBasicRect2.prototype.moveNE(c, x, y, w, h, rectStyle,
                topRightStyle, size, top);
            mxShapeBasicRect2.prototype.paintRight(c, x, y, w, h, rectStyle,
                bottomRightStyle, size, bottom);

            if (rectOutline == 'double') {
                mxShapeBasicRect2.prototype.moveSEInner(c, x, y, w, h,
                    rectStyle, bottomRightStyle, size, indent, bottom);
                mxShapeBasicRect2.prototype.paintRightInner(c, x, y, w, h,
                    rectStyle, topRightStyle, size, indent, top, right);
            }
            ...
            mxShapeBasicRect2.prototype.paintRight(c, x, y, w, h, rectStyle,
                bottomRightStyle, size, bottom);
            mxShapeBasicRect2.prototype.lineSEInner(c, x, y, w, h, rectStyle,
                bottomRightStyle, size, indent, bottom);
            mxShapeBasicRect2.prototype.paintRightInner(c, x, y, w, h,
                rectStyle, topRightStyle, size, indent, top, right);
            c.close();
            c.fillAndStroke();
        ...
        if (rectOutline != 'frame') {
            ...
            if (rectOutline == 'double') {
                ...
                mxShapeBasicRect2.prototype.paintBottomInner(c, x, y, w, h,
                    rectStyle, bottomRightStyle, size, indent, right, bottom);
                mxShapeBasicRect2.prototype.paintSEInner(c, x, y, w, h,
                    rectStyle, bottomRightStyle, size, indent);
                mxShapeBasicRect2.prototype.paintRightInner(c, x, y, w, h,
                    rectStyle, topRightStyle, size, indent, top, right);
            }
        ...
        if (rectOutline != 'frame') {
            ...
            if (rectOutline == 'double') {
                mxShapeBasicRect2.prototype.moveNEInner(c, x, y, w, h,
                    rectStyle, topRightStyle, size, indent, right);
                mxShapeBasicRect2.prototype.paintTopInner(c, x, y, w, h,
                    rectStyle, topLeftStyle, size, indent, left, top);
            }
            ...
            mxShapeBasicRect2.prototype.paintTop(c, x, y, w, h, rectStyle,
                topRightStyle, size, right);
            mxShapeBasicRect2.prototype.lineNEInner(c, x, y, w, h, rectStyle,
                topRightStyle, size, indent, right);
            mxShapeBasicRect2.prototype.paintTopInner(c, x, y, w, h, rectStyle,
                topLeftStyle, size, indent, left, top);
            c.close();
            c.fillAndStroke();
        ...
        if (rectOutline != 'frame') {
            ...
            if (rectOutline == 'double') {
                ...
                mxShapeBasicRect2.prototype.paintTopInner(c, x, y, w, h,
                    rectStyle, topLeftStyle, size, indent, left, top);
                mxShapeBasicRect2.prototype.paintNWInner(c, x, y, w, h,
                    rectStyle, topLeftStyle, size, indent);
                mxShapeBasicRect2.prototype.paintLeftInner(c, x, y, w, h,
                    rectStyle, bottomLeftStyle, size, indent, bottom, left);
            }
        ...
        if (rectOutline != 'frame') {
            ...
            if (rectOutline == 'double') {
                ...
                mxShapeBasicRect2.prototype.paintRightInner(c, x, y, w, h,
                    rectStyle, topRightStyle, size, indent, top, right);
                mxShapeBasicRect2.prototype.paintNEInner(c, x, y, w, h,
                    rectStyle, topRightStyle, size, indent);
                mxShapeBasicRect2.prototype.paintTopInner(c, x, y, w, h,
                    rectStyle, topLeftStyle, size, indent, left, top);
            }
        ...
        if (rectOutline != 'frame') {
            ...
            mxShapeBasicRect2.prototype.paintLeft(c, x, y, w, h, rectStyle,
                topLeftStyle, size, top);
            c.close();
            mxShapeBasicRect2.prototype.moveSWInner(c, x, y, w, h, rectStyle,
                bottomLeftStyle, size, indent, left);
            mxShapeBasicRect2.prototype.paintSWInner(c, x, y, w, h, rectStyle,
                bottomLeftStyle, size, indent, bottom);
            mxShapeBasicRect2.prototype.paintBottomInner(c, x, y, w, h,
    ...
    c.begin();
    mxShapeBasicRect2.prototype.paintFolds(c, x, y, w, h, rectStyle,
        topLeftStyle, topRightStyle, bottomRightStyle, bottomLeftStyle, size,
        top, right, bottom, left);
    c.stroke();
};

mxShapeBasicRect2.prototype.moveNW = function(
    c, x, y, w, h, rectStyle, topLeftStyle, size, left) {
    if ((topLeftStyle == 'square' ||
        (topLeftStyle == 'default' && rectStyle == 'square' )) || !left) {
        c.moveTo(0, 0);
    }
    else {
        c.moveTo(0, size);
...
mxShapeBasicRect2.prototype.moveNE = function(
    c, x, y, w, h, rectStyle, topRightStyle, size, top) {
    if ((topRightStyle == 'square' ||
        (topRightStyle == 'default' && rectStyle == 'square' )) || !top) {
        c.moveTo(w, 0);
    }
    else {
        c.moveTo(w - size, 0);
...
mxShapeBasicRect2.prototype.moveSE = function(
    c, x, y, w, h, rectStyle, bottomRightStyle, size, right) {
    if ((bottomRightStyle == 'square' ||
        (bottomRightStyle == 'default' && rectStyle == 'square' )) || !right) {
        c.moveTo(w, h);
    }
    else {
        c.moveTo(w, h - size);
...
mxShapeBasicRect2.prototype.moveSW = function(
    c, x, y, w, h, rectStyle, bottomLeftStyle, size, bottom) {
    if ((bottomLeftStyle == 'square' ||
        (bottomLeftStyle == 'default' && rectStyle == 'square' )) || !bottom) {
        c.moveTo(0, h);
    }
    else {
        c.moveTo(size, h);
...
mxShapeBasicRect2.prototype.paintNW = function(
    ...
    else if ((topLeftStyle == 'snip' ||
        (topLeftStyle == 'default' && rectStyle == 'snip' )) ||
        (topLeftStyle == 'fold' ||
...
mxShapeBasicRect2.prototype.paintTop = function(
    c, x, y, w, h, rectStyle, topRightStyle, size, right) {
    if ((topRightStyle == 'square' ||
        (topRightStyle == 'default' && rectStyle == 'square' )) || !right) {
        c.lineTo(w, 0);
    }
    else {
        c.lineTo(w - size, 0);
...
mxShapeBasicRect2.prototype.paintRight = function(
    c, x, y, w, h, rectStyle, bottomRightStyle, size, bottom) {
    if ((bottomRightStyle == 'square' ||
        (bottomRightStyle == 'default' && rectStyle == 'square' )) || !bottom) {
        c.lineTo(w, h);
    }
    else {
        c.lineTo(w, h - size);
...
mxShapeBasicRect2.prototype.paintLeft = function(
    c, x, y, w, h, rectStyle, topLeftStyle, size, top) {
    if ((topLeftStyle == 'square' ||
        (topLeftStyle == 'default' && rectStyle == 'square' )) || !top) {
        c.lineTo(0, 0);
    }
    else {
        c.lineTo(0, size);
...
mxShapeBasicRect2.prototype.paintBottom = function(
    c, x, y, w, h, rectStyle, bottomLeftStyle, size, left) {
    if ((bottomLeftStyle == 'square' ||
        (bottomLeftStyle == 'default' && rectStyle == 'square' )) || !left) {
        c.lineTo(0, h);
    }
    else {
        c.lineTo(size, h);
...
mxShapeBasicRect2.prototype.paintNWInner = function(
    c, x, y, w, h, rectStyle, topLeftStyle, size, indent) {
    if (topLeftStyle == 'rounded' ||
        (topLeftStyle == 'default' && rectStyle == 'rounded' )) {
        c.arcTo(size - indent * 0.5, size - indent * 0.5, 0, 0, 0, indent,
            indent * 0.5 + size);
    }
    else if (topLeftStyle == 'invRound' ||
        (topLeftStyle == 'default' && rectStyle == 'invRound' )) {
...
mxShapeBasicRect2.prototype.paintTopInner = function(
    c, x, y, w, h, rectStyle, topLeftStyle, size, indent, left, top) {
    if (!left && !top) {
        c.lineTo(0, 0);
    }
    else if (!left && top) {
    ...
    else if (topLeftStyle == 'square' ||
        (topLeftStyle == 'default' && rectStyle == 'square' )) {
...
mxShapeBasicRect2.prototype.paintNEInner = function(
    c, x, y, w, h, rectStyle, topRightStyle, size, indent) {
    if (topRightStyle == 'rounded' ||
        (topRightStyle == 'default' && rectStyle == 'rounded' )) {
        c.arcTo(size - indent * 0.5, size - indent * 0.5, 0, 0, 0,
            w - size - indent * 0.5, indent);
    }
    else if (topRightStyle == 'invRound' ||
        (topRightStyle == 'default' && rectStyle == 'invRound' )) {
        c.arcTo(size + indent, size + indent, 0, 0, 1, w - size - indent,
            indent);
    }
    else if (topRightStyle == 'snip' ||
        (topRightStyle == 'default' && rectStyle == 'snip' )) {
...
mxShapeBasicRect2.prototype.paintRightInner = function(
    c, x, y, w, h, rectStyle, topRightStyle, size, indent, top, right) {
    if (!top && !right) {
        c.lineTo(w, 0);
    }
    else if (!top && right) {
    ...
    else if (topRightStyle == 'square' ||
        (topRightStyle == 'default' && rectStyle == 'square' )) {
...
mxShapeBasicRect2.prototype.paintLeftInner = function(
    c, x, y, w, h, rectStyle, bottomLeftStyle, size, indent, bottom, left) {
    if (!bottom && !left) {
        c.lineTo(0, h);
    }
    else if (!bottom && left) {
        ...
        (bottomLeftStyle == 'default' && rectStyle == 'square' )) {
...
mxShapeBasicRect2.prototype.paintSEInner = function(
    c, x, y, w, h, rectStyle, bottomRightStyle, size, indent) {
    if (bottomRightStyle == 'rounded' ||
        (bottomRightStyle == 'default' && rectStyle == 'rounded' )) {
        c.arcTo(size - indent * 0.5, size - indent * 0.5, 0, 0, 0, w - indent,
            h - size - indent * 0.5);
    }
    else if (bottomRightStyle == 'invRound' ||
        (bottomRightStyle == 'default' && rectStyle == 'invRound' )) {
        c.arcTo(size + indent, size + indent, 0, 0, 1, w - indent,
            h - size - indent);
    }
    else if (bottomRightStyle == 'snip' ||
        (bottomRightStyle == 'default' && rectStyle == 'snip' )) {
...
mxShapeBasicRect2.prototype.paintBottomInner = function(
    c, x, y, w, h, rectStyle, bottomRightStyle, size, indent, right, bottom) {
    if (!right && !bottom) {
        c.lineTo(w, h);
    }
    ...
    else if ((bottomRightStyle == 'square' ||
        (bottomRightStyle == 'default' && rectStyle == 'square' )) || !right) {
...
mxShapeBasicRect2.prototype.paintSWInner = function(
    c, x, y, w, h, rectStyle, bottomLeftStyle, size, indent, bottom) {
    if (!bottom) {
        c.lineTo(indent, h);
    }
    else if (bottomLeftStyle == 'square' ||
        ...
        (bottomLeftStyle == 'default' && rectStyle == 'rounded' )) {
        c.arcTo(size - indent * 0.5, size - indent * 0.5, 0, 0, 0,
            size + indent * 0.5, h - indent);
    }
    else if (bottomLeftStyle == 'invRound' ||
        (bottomLeftStyle == 'default' && rectStyle == 'invRound' )) {
        c.arcTo(size + indent, size + indent, 0, 0, 1, size + indent,
            h - indent);
    }
    else if (bottomLeftStyle == 'snip' ||
        (bottomLeftStyle == 'default' && rectStyle == 'snip' )) {
...
mxShapeBasicRect2.prototype.moveSWInner = function(
    c, x, y, w, h, rectStyle, bottomLeftStyle, size, indent, left) {
    if (!left) {
        c.moveTo(0, h - indent);
    }
    else if (bottomLeftStyle == 'square' ||
...
mxShapeBasicRect2.prototype.lineSWInner = function(
    c, x, y, w, h, rectStyle, bottomLeftStyle, size, indent, left) {
    if (!left) {
        c.lineTo(0, h - indent);
    }
...
mxShapeBasicRect2.prototype.moveSEInner = function(
    c, x, y, w, h, rectStyle, bottomRightStyle, size, indent, bottom) {
    if (!bottom) {
        c.moveTo(w - indent, h);
    }
    else if (bottomRightStyle == 'square' ||
        (bottomRightStyle == 'default' && rectStyle == 'square' )) {
...
mxShapeBasicRect2.prototype.lineSEInner = function(
    c, x, y, w, h, rectStyle, bottomRightStyle, size, indent, bottom) {
    if (!bottom) {
        c.lineTo(w - indent, h);
    }
...
mxShapeBasicRect2.prototype.moveNEInner = function(
    c, x, y, w, h, rectStyle, topRightStyle, size, indent, right) {
    if (!right) {
        c.moveTo(w, indent);
    }
    else if ((topRightStyle == 'square' ||
...
mxShapeBasicRect2.prototype.lineNEInner = function(
    c, x, y, w, h, rectStyle, topRightStyle, size, indent, right) {
    if (!right) {
        c.lineTo(w, indent);
    }
...
mxShapeBasicRect2.prototype.moveNWInner = function(
...
mxShapeBasicRect2.prototype.lineNWInner = function(
    c, x, y, w, h, rectStyle, topLeftStyle, size, indent, top, left) {
    if (!top && !left) {
        c.lineTo(0, 0);
    }
...
mxShapeBasicRect2.prototype.paintFolds = function(
    c, x, y, w, h, rectStyle, topLeftStyle, topRightStyle, bottomRightStyle,
    bottomLeftStyle, size, top, right, bottom, left) {
    if (rectStyle == 'fold' || topLeftStyle == 'fold' ||
        topRightStyle == 'fold' || bottomRightStyle == 'fold' ||
        bottomLeftStyle == 'fold') {
        if ((topLeftStyle == 'fold' ||
            (topLeftStyle == 'default' && rectStyle == 'fold' )) &&
            (top && left)) {
            c.moveTo(0, size);
            c.lineTo(size, size);
...
mxCellRenderer.registerShape(mxShapeBasicRect2.prototype.cst.RECT2,
    mxShapeBasicRect2);
...
Graph.handleFactory[mxShapeBasicRect2.prototype.cst.DIAG_ROUND_RECT] = function(state) {
    ...
    var handle2 = Graph.createHandle(state, ['indent'], function(bounds) {
        var dx2 = Math.max(0, Math.min(100, parseFloat(
            mxUtils.getValue(this.state.style, 'indent', this.dx2))));

        return new mxPoint(bounds.x + bounds.width * 0.75,
            bounds.y + dx2 * bounds.height / 200);
    }, function(bounds, pt) {
        this.state.style['indent'] = Math.round(100 * Math.max(0,
                    Math.min(100, 200 * (pt.y - bounds.y) / bounds.height))) /
            100;
    });
