...
(function()
{
	...
	EditorUi.prototype.getHtml = function(node, graph, title, editLink, redirect, ignoreSelection)
	{
		ignoreSelection = (ignoreSelection != null) ? ignoreSelection : true;
		var bg = null;
		var js = 'http://' + window.location.host + '/js/embed-static.min.js';
		...
		if (graph != null)
		{
			...
			if (redirect == null)
			{
				...
				if (s.length > 0)
				{
					js = 'http://' + window.location.host + '/embed.js?s=' + s;
				}
			}
		...
		return ((redirect == null) ? '<!--[if IE]><meta http-equiv="X-UA-Compatible" content="IE=5,IE=9" ><![endif]-->\n' : '') +
			...
			'src="http://'+window.location.host+'/images/drawlogo128.png"/></a>') +
			'\n</body>\n</html>\n';
	};
	...
	{
		var bg = null;
		var js = 'http://' + window.location.host + '/js/viewer.min.js';
		var s = '';
		...
		return ((redirect == null) ? '<!--[if IE]><meta http-equiv="X-UA-Compatible" content="IE=5,IE=9" ><![endif]-->\n' : '') +
			...
			'<a style="position:absolute;top:50%;left:50%;margin-top:-128px;margin-left:-64px;" ' +
			'href="' + redirect + '" target="_blank"><img border="0" ' +
			'src="https://'+window.location.host+'/images/drawlogo128.png"/></a>') +
			'\n</body>\n</html>\n';
	};
		...
		function doUpdate(dataUri)
		{
			...
			if (lightbox)
			{
				...
				// 	"img.getAttribute('src')),'*');window.removeEventListener('message',r);}};window.addEventListener('message',r);img.wnd=window.open('https://www.draw.io/?client=1&lightbox=1" +
					onclick = " onclick=\"(function(img){if(img.wnd!=null&&!img.wnd.closed){img.wnd.focus();}else{var r=function(evt){if(evt.data=='ready'&&evt.source==img.wnd){img.wnd.postMessage(decodeURIComponent(" +
						"img.getAttribute('src')),'*');window.removeEventListener('message',r);}};window.addEventListener('message',r);img.wnd=window.open('http://" + window.location.host + "/?client=1&lightbox=1" +
					((edit) ? "&edit=_blank" : "") +
		...
		else
		{
			...
			if (lightbox)
			{
				...
				var js = "(function(svg){var src=window.event.target||window.event.srcElement;" +
					...
					"svg.wnd=window.open('http://"+window.location.host+"/?client=1&lightbox=1" +
					((edit) ? "&edit=_blank" : "") + ((layers) ? '&layers=1' : '') + "');}}})(this);";
...
//		}
								...
								if (provider != null)
								{
									...
									if (data != null)
									{
										...
										else if (/^https?:\/\//.test(data))
										{
											...
											{
												window.openWindow('http://' + window.location.host + '/' + window.location.search + '#U' + encodeURIComponent(data));
											}
										}
									}
								}
	...
	EditorUi.prototype.updateActionStates = function()
	{
		...
		this.actions.get('searchByAttribute').setEnabled(this.diagramContainer.style.visibility != 'hidden');
