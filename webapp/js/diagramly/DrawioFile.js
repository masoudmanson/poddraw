...
DrawioFile.prototype.mergeFile = function(file, success, error) {
    try {
        ...
        var shadow = (this.shadowPages != null) ? this.shadowPages : this.ui.getPagesForNode(mxUtils.parseXml(
...
DrawioFile.prototype.getChannelId = function() {
    return this.ui.editor.graph.compress(this.getHash())
        .replace(/\//g, '-')
        .replace(/ /g, '_');
};
        ...
        if (this.constructor == DriveFile || this.constructor == DropboxFile) {
            ...
            if (links.length > 0) {
                mxEvent.addListener(links[0], 'click', mxUtils.bind(this, function() {
                    this.ui.actions.get('revisionHistory')
                        .funct();
                }));
            }
        }
    ...
    if (!this.inConflictState && this.ui.statusContainer != null) {
        if (err instanceof Error && err.message != null) {
            ...
            if (links != null && links.length > 0) {
                mxEvent.addListener(links[0], 'click', mxUtils.bind(this, function() {
                    this.ui.actions.get((this.ui.mode == null || !this.isEditable()) ? 'saveAs' : 'save')
                        .funct();
                }));
            }
            else {
