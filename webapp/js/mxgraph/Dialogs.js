...
var OpenFromDatabaseDialog = function() {
    var iframe = document.createElement('iframe');
    iframe.style.backgroundColor = 'transparent';
    iframe.allowTransparency = 'true';
    iframe.style.borderStyle = 'none';
    ...
    iframe.setAttribute('width', (((Editor.useLocalStorage) ? 640 : 320) + dx) + 'px');
    iframe.setAttribute('height', (((Editor.useLocalStorage) ? 480 : 220) + dx) + 'px');
    iframe.setAttribute('src', OPEN_FORM_DATABASE);
 ...
 * Constructs a new filename dialog.
 */
var MapsListFromDatabaseDialog = function(editorUi, content) {
    var div = document.createElement('div');
    div.style.width = "100%";
    var h3 = document.createElement('h3');
    mxUtils.write(h3, mxResources.get('getListFromDatabase') + ':');
    div.appendChild(h3);
    ...
    for (var i = 0; i < content.length; i++) {
        ...
        tempTd.style.whiteSpace = 'nowrap';
        tempTd.style.fontSize = '10pt';
        tempTd.style.width = "100%";
        tempTd.style.padding = "5px";

        console.log(content[i]);
        var tempContent = content[i].content;
        var tempName = content[i].name;
        mxUtils.link(tempTd, content[i].name, function() {
            console.log(tempName);
            console.log(tempContent);
            // editorUi.loadFile();
            // console.log('This is some serious shit happening around!', content[i].content);
            // TODO: save this shit on database
            // console.log(editorUi.getFileData(true, null, null, null, true, false));
        }, 0);
...
ExportDialog.exportFile = function(editorUi, name, format, bg, s, b) {
    ...
    if (format == 'xml') {
        ...
        imgExport.drawState(graph.getView()
            .getState(graph.model.root), xmlCanvas);
...
var EditDataDialog = function(ui, cell) {
    ...
    var addRemoveButton = function(text, name, index) {
        ...
        removeAttr.setAttribute('data-delete', index);
        ...
        var removeAttrFn = (function(name) {
            return function() {
                texts[index] = null;
                form.table.deleteRow(this.parentNode.parentNode.parentNode.rowIndex);
    ...
    var addTextArea = function(index, name, value) {
        ...
        addRemoveButton(texts[index], name, index);
    };
    ...
    var addComboBox = function(index, name, value) {
        oDataRequest({
            ...
            if (!result.hasError) {
                names[index] = name;
                texts[index] = form.addCombo(name + ':', false, 1);
                texts[index].style.width = '100%';
                addRemoveButton(texts[index], name, index);
                ...
                for (var i = 0; i < tags.length; i++) {
                    form.addOption(texts[index], tags[i].tagname, tags[i].tagname, ((tags[i].tagname == value) ? true : false));
                }
            }
            else {
    ...
    for (var i = 0; i < temp.length; i++) {
        if (mxUtils.indexOf(reservedNames, temp[i].name) >= 0) {
            addComboBox(count, temp[i].name, temp[i].value);
        }
        else {
                    ...
                    if (mxUtils.indexOf(reservedNames, name) >= 0) {
                        oDataRequest({
                            ...
                            if (!result.hasError) {
                                ...
                                for (var i = 0; i < tags.length; i++) {
                                    form.addOption(text, tags[i].tagname, tags[i].tagname, ((i == 0) ? true : false));
                                }
                            }
                            else {
    ...
    if (ui.editor.graph.getModel()
            .isVertex(cell) || ui.editor.graph.getModel()
            .isEdge(cell)) {
        ...
        function addLayer(index, label, child, defaultParent) {
            ...
            mxEvent.addListener(btn, 'click', function(evt) {
                if (graph.isEnabled()) {
                    ...
                    if (value == '1') {
                        graph.removeSelectionCells(graph.getModel()
