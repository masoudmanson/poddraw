...
EditorUi = function(editor, container, lightbox) {
    ...
    var alwaysEdgeStyles = [
        'edgeStyle', 'startArrow', 'startFill', 'startSize', 'endArrow',
    ...
    this.addListener('styleChanged', mxUtils.bind(this, function(sender, evt) {
        ...
        if (cells.length > 0) {
            for (var i = 0; i < cells.length; i++) {
                vertex = graph.getModel()
                        .isVertex(cells[i]) || vertex;
                edge = graph.getModel()
                        .isEdge(cells[i]) || edge;
        ...
        if (this.toolbar != null) {
            ...
            if (this.toolbar.edgeStyleMenu != null) {
                ...
                if (graph.currentEdgeStyle['edgeStyle'] == 'orthogonalEdgeStyle' && graph.currentEdgeStyle['curved'] == '1') {
                    ...
                    edgeStyleDiv.className = 'geSprite geSprite-' +
    ...
    if (this.toolbar != null) {
        var update = mxUtils.bind(this, function() {
            ...
            var state = graph.getView()
                .getState(graph.getSelectionCell());
        ...
        graph.getSelectionModel()
            .addListener(mxEvent.CHANGE, update);
        graph.getModel()
            .addListener(mxEvent.CHANGE, update);
    }
    ...
    graph.addListener(mxEvent.CELLS_ADDED, function(sender, evt) {
        ...
        if (graph.getModel()
                .isLayer(parent) && !graph.isCellVisible(parent) && cells != null && cells.length > 0) {
            graph.getModel()
...
EditorUi.prototype.init = function() {
    ...
    graph.getSelectionModel()
        .addListener(mxEvent.CHANGE, mxUtils.bind(this, function() {
...
EditorUi.prototype.updateActionStates = function() {
    ...
    // Updates action states
    var actions = [
        'cut', 'copy', 'bold', 'italic', 'underline', 'delete', 'duplicate',
    ...
    this.actions.get('setAsDefaultStyle')
        .setEnabled(graph.getSelectionCount() == 1);
    this.actions.get('clearWaypoints')
        .setEnabled(!graph.isSelectionEmpty());
    this.actions.get('copySize')
        .setEnabled(graph.getSelectionCount() == 1);
    this.actions.get('turn')
        .setEnabled(!graph.isSelectionEmpty());
    ...
    this.actions.get('guides')
        .setEnabled(graph.isEnabled());
    this.actions.get('grid')
        .setEnabled(!this.editor.chromeless || this.editor.editable);

    var unlocked = graph.isEnabled() && !graph.isCellLocked(graph.getDefaultParent());
    this.menus.get('layout')
        .setEnabled(unlocked);
    this.menus.get('insert')
        .setEnabled(unlocked);
    this.menus.get('direction')
        .setEnabled(unlocked && vertexSelected);
    this.menus.get('align')
        .setEnabled(unlocked && vertexSelected && graph.getSelectionCount() > 1);
    this.menus.get('distribute')
        .setEnabled(unlocked && vertexSelected && graph.getSelectionCount() > 1);
    this.actions.get('selectVertices')
        .setEnabled(unlocked);
    this.actions.get('selectEdges')
        .setEnabled(unlocked);
    this.actions.get('selectAll')
        .setEnabled(unlocked);
    this.actions.get('selectNone')
        .setEnabled(unlocked);
...
EditorUi.prototype.pickColor = function(color, apply) {
    ...
    var h = 236 + ((Math.ceil(ColorDialog.prototype.presetColors.length / 12) +
    ...
    this.showDialog(dlg.container, 240, h, true, false);
    dlg.init();
};
 ...
 */
EditorUi.prototype.openFile = function(mode) {
    // Closes dialog after open
    window.openFile = new OpenFile(mxUtils.bind(this, function(cancel) {
        this.hideDialog(cancel);
    }));

    if (mode == 'fromDatabase') {
        // Removes openFile if dialog is closed
        this.showDialog(new OpenFromDatabaseDialog(this).container, 640, 480, true, true, function() {
...
EditorUi.prototype.extractGraphModelFromHtml = function(data) {
    ...
    try {
        ...
        if (idx >= 0) {
            ...
            if (idx2 > idx) {
                result = data.substring(idx, idx2 + 21)
                    .replace(/&gt;/g, '>')
                    .replace(/&lt;/g, '<')
                    .replace(/\\&quot;/g, '"')
                    .replace(/\n/g, '');
            ...
            if (!graph.isSelectionEmpty() && graph.isEnabled()) {
                ...
                if (resize) {
                    ...
                    try {
                        ...
                        for (var i = 0; i < cells.length; i++) {
                            if (graph.getModel()
                                    .isVertex(cells[i]) && graph.isCellResizable(cells[i])) {
...
EditorUi.prototype.destroy = function() {
    ...
    var c = [
        this.menubarContainer, this.toolbarContainer, this.sidebarContainer,
